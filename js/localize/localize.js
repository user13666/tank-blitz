...
(() => {
  ...
  Array.prototype.map.call(data, elementDom => {
    const dataLocalizeValue = elementDom.getAttribute('data-localize').toString();
    const filterValue = replace_i18n(dataLocalizeValue).toString();
...
function replace_i18n(dataLocalizeValue) {
  const msg = dataLocalizeValue.replace(/__MSG_(\w+)__/g, (value, filterValue) => {
...
const getLocalizeText = (filterValue, defaultText = '') =>
  chrome.i18n.getMessage(filterValue) !== '' ? chrome.i18n.getMessage(filterValue) : defaultText;
